...
resource "aws_route" "public-world" {
  ...
  destination_cidr_block = "0.0.0.0/0" #local.net.world
...
resource "aws_subnet" "dmz" {
  ...
  cidr_block        = cidrsubnet("10.254.0.0/16", 1, count.index)
}
...
resource "aws_ec2_client_vpn_endpoint" "the_vpn" {
  server_certificate_arn = "arn:aws:acm:us-east-1:XXXXXXXXXXXXX:certificate/XXXXXXXXXXXXXXXXX" 
